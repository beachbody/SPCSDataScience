...
# Sub heading info
st.markdown(
    "This app is developed to go off of the account usage schema in your Snowflake account. For detailed information please see the documentation page below."
)
st.markdown(
...
# Info bar
st.info(
    "Developed by the Snowflake Sales Engineering Team. This is not a product of Snowflake please use at your own risk.",
    icon="ℹ️",
)
st.divider()
...
with col1:
    if st.button("30 Days"):
        st.session_state.starting = datetime.datetime.now() - datetime.timedelta(
            days=30
        )
...
fig_compute_gs_by_warehouse = px.bar(
    compute_gs_by_warehouse_df,
    x="WAREHOUSE_NAME",
...
fig_logins_client = px.bar(
    logins_client_df, x="CLIENT", y="SUCCESS", orientation="v", title="Logins by Client"
)
